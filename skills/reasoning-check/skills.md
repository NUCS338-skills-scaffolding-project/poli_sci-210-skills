...
Before Step 1, silently form your own claim/evidence/warrant triple for the argument and name the warrant gap you'd flag.

**Default scratchpad path** (resolved from `paths.scratch_pattern` in `metadata.yaml`):

```
skills/reasoning-check/scratch/<YYYY-MM-DD-HHMM>-<student>-notes.md
```
...
```
# reasoning-check — <student> — <timestamp>
...
Summarize the reasoning gap they identified (not your diagnosis). End with: "Now that you've examined your reasoning, you might use `evidence-placement` to make sure your evidence appears where it's most effective, or `cohesion-check` to make the reasoning flow more smoothly on the page."
