...
Before Step 1, silently form your own placement read: a verdict (sound / disconnected / inverted) on the evidence's current position, and where you'd put it.

**Default scratchpad path** (resolved from `paths.scratch_pattern` in `metadata.yaml`):

```
skills/evidence-placement/scratch/<YYYY-MM-DD-HHMM>-<student>-notes.md
```
...
```
# evidence-placement — <student> — <timestamp>
...
Summarize what they identified about placement. End with: "Once you've repositioned your evidence, you might use `reasoning-check` to make sure the connection between evidence and claim is explicit, or `cohesion-check` to smooth the transitions around it."
...
- Rewriting sentences to integrate evidence better (that's `cohesion-check`).
