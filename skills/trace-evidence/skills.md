...
## Description
Granular skill in Phase 1 (`orient-paper`) of the research design critique. Forces the student to pick the paper's headline empirical claim, then locate the specific table, figure, or passage that supports it, then read what's actually there. The pedagogical move is *reading past the framing into the guts*: students often grasp the abstract but never look at the central table. This skill closes that gap.

## When to Trigger
- Opened by the `orient-paper` orchestrator as the second skill in its chain (after `first-pass-orient`).
...
>
> **Tutor:** That's the trace. *(writes Completion Notes; phase orchestrator advances to author-choices)*
