...
- Work at the section/paragraph level, not sentence level (that's `cohesion-check`).
...
Before Step 1, silently form your own read of the section-to-section transitions: for each adjacent pair, name the kind of move (continuation, contrast, escalation, sequencing, etc.).

**Default scratchpad path** (resolved from `paths.scratch_pattern` in `metadata.yaml`):

```
skills/flow-check/scratch/<YYYY-MM-DD-HHMM>-<student>-notes.md
```
...
Summarize what they identified (not what you think). End with: "Once you've addressed this, you might also look at `cohesion-check` to tighten the connections within sections, or `reasoning-check` if you want to test whether your evidence-to-claim logic holds."
...
- Working at the sentence level (that's `cohesion-check`).
- Evaluating the quality of their reasoning (that's `reasoning-check`).
