...
4. Set the **Owner** to the Organization _(not your personal account)_
...
- `skills_used` is a list of skills you've adopted from _other_ teams' repos. Yours go under `/skills/` and are picked up automatically.
...
- Text analysis _(scanning student essays for tone or complexity)_
- Response scoring _(checking if answers hit key concepts)_
- Prompt generation _(dynamically generating Socratic questions)_
- Keyword detection _(flagging when a student is off topic)_
- Progress tracking _(logging how far a student is through a skill)_

**Example skills:**

- _Bound Scope_ — helping students avoid overengineering (no logic.py needed)
- _Counter-Example_ — pressure-testing a claim with a probe (no logic.py needed)
- _Off-Topic Detector_ — flags a drifting essay (would have a logic.py)
...
- _Sorting Algorithm_ — reusable sort function
- _Input Validator_ — checks and cleans user input
- _Graph Traversal_ — BFS/DFS implementation
...
If your instructional skill _does_ need helpers (text analysis, scoring, etc.), include `logic.py` with the same `run(input)` shape as code skills. The orchestrator will call it as a helper alongside running the `skills.md` flow.
...
Every instructional skill declares a `stance` in its YAML header. The stance tells the orchestrator and the tutor _how_ this skill talks to the student. There are four:
...
| **`reframe`**  | The system restates the situation in different terms (analogy, concrete trace, alternative angle) to unstick a stuck framing.                | Your skill changes _how_ the student is thinking about the problem, without giving the answer or asking a new question. | `reframe-with-analogy`, `reframe-with-trace`                                            |
...
If your skill seems to fit two stances, pick the one that describes its _primary_ move — the one a student would notice first.
